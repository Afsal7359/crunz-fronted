...
const TEXT_FIELDS = [
  ...
  { key: 'hero_eyebrow',   label: 'Hero Eyebrow (left text)',   hint: 'Small label above title e.g. Premium Banana Chips · Preston, UK' },
  { key: 'hero_title',     label: 'Hero Title (left text)',     hint: 'Main headline e.g. CRUNZ' },
  { key: 'hero_sub',       label: 'Hero Subtitle (left text)',  hint: 'Short tagline below title' },
...
// ── Reusable slide manager (used by BOTH left and right panels) ────
...
export default function ContentAdmin() {
  ...
  const [leftSlides, setLeftSlides] = useState([]);   // hero LEFT panel slides
  const [rightSlides, setRightSlides] = useState([]); // hero RIGHT panel slides
  ...
  useEffect(() => {
    api.get('/admin/content').then(d => {
      setContent(d);
      // Left slides: try new key first, fall back to old hero_left_images format
      try {
        if (d.hero_left_slides) {
          setLeftSlides(JSON.parse(d.hero_left_slides));
        } else if (d.hero_left_images) {
          const old = JSON.parse(d.hero_left_images);
          setLeftSlides(old.map(img => ({ type: 'image', src: img.src, label: img.label || '' })));
        }
      } catch { setLeftSlides([]); }
      // Right slides
      try { setRightSlides(d.hero_slides ? JSON.parse(d.hero_slides) : []); } catch { setRightSlides([]); }
  ...
  const handleSave = async () => {
    ...
    try {
      await api.put('/admin/content', {
        ...content,
        hero_left_slides:  JSON.stringify(leftSlides),
        hero_slides:       JSON.stringify(rightSlides),
      });
      setSaved(true);
  ...
  return (
    <div>
      ...
      <SlideManager
        slides={leftSlides}
        setSlides={setLeftSlides}
        title="Banner Left — Slides"
        hint="Add image or text slides for the LEFT side of the banner. Multiple slides scroll infinitely. If empty, the text block (title/subtitle/CTA below) is shown instead."
        emptyNote="No left slides — the text (eyebrow, title, subtitle, Shop Now button) is shown on the left. Click '+ Image Slide' or '+ Text Slide' to override with custom slides."
        onSave={handleSave}
        saving={saving}
        saved={saved}
        saveLabel="Save Left Slides"
      />

      {/* ── 2. Hero Left Text Fields (fallback when no slides) ────── */}
      <div className="admin-form-wrap" style={{ marginBottom: 20 }}>
        <div style={{ fontWeight: 800, fontSize: '1rem', letterSpacing: '-.4px', marginBottom: 4 }}>Banner Left — Text (fallback)</div>
        <div style={{ fontSize: '.78rem', opacity: .45, marginBottom: 18 }}>Shown on the left only when no slides are added above.</div>
      ...
      <SlideManager
        slides={rightSlides}
        setSlides={setRightSlides}
        title="Banner Right — Slides"
        hint="Image or text slides for the RIGHT side of the banner. Auto-scrolls every 3 s. If empty, product images are used."
        emptyNote="No custom slides — product images are used by default."
        onSave={handleSave}
        saving={saving}
        saved={saved}
        saveLabel="Save Right Slides"
      />
