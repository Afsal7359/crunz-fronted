...
/* ── Out-of-stock block (desktop + mobile) ── */
function OOSBlock({ wa = '447741940700', productName = '', mobile = false }) {
  const waNum = wa || '447741940700';
  const msg = encodeURIComponent(`Hi! I'm interested in "${productName}" but it's showing Out of Stock. Can you let me know when it's back?`);
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{
        ...
        <div style={{ fontWeight: 800, fontSize: '.9rem', color: '#dc2626', marginBottom: 4 }}>
          Out of Stock
        </div>
        <div style={{ fontSize: '.82rem', color: '#666', lineHeight: 1.6 }}>
          This product is temporarily unavailable. Reach out on WhatsApp and we'll notify you when it's back!
        </div>
      </div>
      <a
        ...
        style={{
          ...
          padding: mobile ? '14px 10px' : '15px 28px',
          fontSize: mobile ? '.95rem' : '1rem', fontWeight: 700,
  ...
  return (
    <>
        ...
        <section style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 5% 56px' }}>
          ...
          >
            ...
            <div style={{
              ...
              {product.inStock === false ? (
                <div style={{
                  ...
                  padding: '5px 12px', borderRadius: 5, letterSpacing: 1, textTransform: 'uppercase',
                }}>Out of Stock</div>
              ) : product.badge && (
                <div style={{
                  position: 'absolute', top: 20, left: 20, zIndex: 2,
              ...
              <img
                src={fixImageUrl(product.image) || '/images/spanish-tomato.jpg'}
                alt={product.name}
                style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', opacity: product.inStock === false ? 0.6 : 1, filter: product.inStock === false ? 'grayscale(30%)' : 'none' }}
              />
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              ...
              <div style={{ marginBottom: 28, display: product.inStock === false ? 'none' : undefined }}>
              ...
              <div className="pd-cta-desktop" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {product.inStock === false ? (
                  <OOSBlock wa={content.whatsapp} productName={product.name} />
                ) : (
                  <>
                    <button
      ...
      <div className="pd-sticky-bar">
        {/* Qty + price row — hidden when out of stock */}
        <div style={{ display: product.inStock === false ? 'none' : 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        {/* Action buttons row */}
        {product.inStock === false ? (
          <OOSBlock wa={content.whatsapp} productName={product.name} mobile />
        ) : (
